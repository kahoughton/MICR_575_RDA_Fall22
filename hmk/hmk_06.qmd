...
1.  Instead of summarizing the conditional distribution with a boxplot, you could use a frequency ploygon. What do you need to consider when using cut_width() vs cut_number()? How does that impact a visualization of the 2d distribution of carat and price?
    ...
    **Answer**: I could not get the freqpoly to work with price as the Y, I kept getting the error that I cannot specify a Y for this geom. So I did just carrot and the cut_number only produced one line along the graph, while the cut_width produced several lines.
